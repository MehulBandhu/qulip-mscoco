...
echo "node $(hostname) | vqc10k_gen | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_gen.yaml

CKPT=$(ls -t checkpoints/mscoco-vqc10k-gen/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_gen.yaml -cp "$CKPT"
echo "finished $(date)"
