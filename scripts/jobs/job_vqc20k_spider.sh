#!/bin/bash
#SBATCH --job-name=q-vqc20k_spider
...
echo "node $(hostname) | vqc20k_spider | $(date)"
python -u -m scripts.train -cfg configs/vqc20k_spider.yaml

CKPT=$(ls -t checkpoints/mscoco-$tag/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/vqc20k_spider.yaml -cp "$CKPT"
echo "finished $(date)"
