#!/bin/bash
#SBATCH --job-name=q-vqc10k_gendrop
...
echo "node $(hostname) | vqc10k_gendrop | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_gendrop.yaml

CKPT=$(ls -t checkpoints/mscoco-vqc10k-gendrop/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_gendrop.yaml -cp "$CKPT"
echo "finished $(date)"
