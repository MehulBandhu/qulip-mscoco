#!/bin/bash
#SBATCH --job-name=q-vqc10k_nowarp
...
echo "node $(hostname) | vqc10k_nowarp | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_nowarp.yaml

CKPT=$(ls -t checkpoints/mscoco-vqc10k-nowarp/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_nowarp.yaml -cp "$CKPT"
echo "finished $(date)"
