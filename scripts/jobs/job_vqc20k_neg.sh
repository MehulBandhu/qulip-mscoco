...
echo "node $(hostname) | vqc20k_neg | $(date)"
python -u -m scripts.train -cfg configs/vqc20k_neg.yaml

CKPT=$(ls -t checkpoints/mscoco-$tag/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/vqc20k_neg.yaml -cp "$CKPT"
echo "finished $(date)"
