#!/bin/bash
#SBATCH --job-name=q-vqc10k_fast5
...
echo "node $(hostname) | vqc10k_fast5 | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_fast5.yaml

CKPT=$(ls -t checkpoints/mscoco-vqc10k-fast5/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_fast5.yaml -cp "$CKPT"
echo "finished $(date)"
