...
echo "node $(hostname) | vqc5k | $(date)"
python -u -m scripts.train -cfg configs/vqc5k.yaml

CKPT=$(find checkpoints/mscoco-vqc5k -name best.pt | head -1)
python -u -m scripts.benchmark -cfg configs/vqc5k.yaml -cp "$CKPT"
echo "finished $(date)"
