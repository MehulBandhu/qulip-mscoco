...
echo "node $(hostname) | vqc10k_l3 | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_l3.yaml

CKPT=$(find checkpoints/mscoco-vqc10k_l3 -name best.pt | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_l3.yaml -cp "$CKPT"
echo "finished $(date)"
