#!/bin/bash
#SBATCH --job-name=q-vqc10k_bs512
...
echo "node $(hostname) | vqc10k_bs512 | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_bs512.yaml

CKPT=$(ls -t checkpoints/mscoco-vqc10k-bs512/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_bs512.yaml -cp "$CKPT"
echo "finished $(date)"
