...
echo "node $(hostname) | tn10k_small | $(date)"
python -u -m scripts.train -cfg configs/tn10k_small.yaml

CKPT=$(ls -t checkpoints/mscoco-$tag/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/tn10k_small.yaml -cp "$CKPT"
echo "finished $(date)"
