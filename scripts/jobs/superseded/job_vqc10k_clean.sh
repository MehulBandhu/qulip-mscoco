#!/bin/bash
#SBATCH --job-name=q-vqc10k_clean
...
echo "node $(hostname) | vqc10k_clean | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_clean.yaml

CKPT=$(ls -t checkpoints/mscoco-vqc10k-clean/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_clean.yaml -cp "$CKPT"
echo "finished $(date)"
