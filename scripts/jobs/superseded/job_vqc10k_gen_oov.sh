#!/bin/bash
#SBATCH --job-name=q-vqc10k_gen_oov
...
echo "node $(hostname) | vqc10k_gen_oov | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_gen_oov.yaml

CKPT=$(ls -t checkpoints/mscoco-vqc10k-gen_oov/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_gen_oov.yaml -cp "$CKPT"
echo "finished $(date)"
