#!/bin/bash
#SBATCH --job-name=q-vqc10k_spider
...
echo "node $(hostname) | vqc10k_spider | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_spider.yaml

CKPT=$(find checkpoints/mscoco-vqc10k_spider -name best.pt | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_spider.yaml -cp "$CKPT"
echo "finished $(date)"
