#!/bin/bash
#SBATCH --job-name=q-vqc10k_nopurity
...
echo "node $(hostname) | vqc10k_nopurity | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_nopurity.yaml

CKPT=$(ls -t checkpoints/mscoco-vqc10k-nopurity/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_nopurity.yaml -cp "$CKPT"
echo "finished $(date)"
