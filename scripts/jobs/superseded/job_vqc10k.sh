...
echo "node $(hostname) | vqc10k | $(date)"
python -u -m scripts.train -cfg configs/vqc10k.yaml

CKPT=$(find checkpoints/mscoco-vqc10k -name best.pt | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k.yaml -cp "$CKPT"
echo "finished $(date)"
