...
echo "node $(hostname) | vqc10k_n2 | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_n2.yaml

CKPT=$(find checkpoints/mscoco-vqc10k_n2 -name best.pt | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_n2.yaml -cp "$CKPT"
echo "finished $(date)"
