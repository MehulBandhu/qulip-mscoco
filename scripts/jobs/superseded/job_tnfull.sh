...
echo "node $(hostname) | tnfull | $(date)"
python -u -m scripts.train -cfg configs/tnfull.yaml

CKPT=$(find checkpoints/mscoco-tnfull -name best.pt | head -1)
python -u -m scripts.benchmark -cfg configs/tnfull.yaml -cp "$CKPT"
echo "finished $(date)"
