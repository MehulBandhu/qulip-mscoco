#!/bin/bash
#SBATCH --job-name=q-tn10k_spider
...
echo "node $(hostname) | tn10k_spider | $(date)"
python -u -m scripts.train -cfg configs/tn10k_spider.yaml

CKPT=$(find checkpoints/mscoco-tn10k_spider -name best.pt | head -1)
python -u -m scripts.benchmark -cfg configs/tn10k_spider.yaml -cp "$CKPT"
echo "finished $(date)"
