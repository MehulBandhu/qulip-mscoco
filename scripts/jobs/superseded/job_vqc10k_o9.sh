...
echo "node $(hostname) | vqc10k_o9 | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_o9.yaml

CKPT=$(ls -t checkpoints/mscoco-vqc10k-o9/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_o9.yaml -cp "$CKPT"
echo "finished $(date)"
