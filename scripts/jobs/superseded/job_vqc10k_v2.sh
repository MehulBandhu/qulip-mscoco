...
echo "node $(hostname) | vqc10k_v2 | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_v2.yaml

CKPT=$(ls -t checkpoints/mscoco-vqc10k-v2/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_v2.yaml -cp "$CKPT"
echo "finished $(date)"
