...
echo "node $(hostname) | tn10k | $(date)"
python -u -m scripts.train -cfg configs/tn10k.yaml

CKPT=$(find checkpoints/mscoco-tn10k -name best.pt | head -1)
python -u -m scripts.benchmark -cfg configs/tn10k.yaml -cp "$CKPT"
echo "finished $(date)"
