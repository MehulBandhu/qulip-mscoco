...
echo "node $(hostname) | vqc10k_k5 | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_k5.yaml

CKPT=$(ls -t checkpoints/mscoco-vqc10k-k5/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_k5.yaml -cp "$CKPT"
echo "finished $(date)"
