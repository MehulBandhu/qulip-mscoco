#!/bin/bash
#SBATCH --job-name=q-vqc10k_nosmooth
...
echo "node $(hostname) | vqc10k_nosmooth | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_nosmooth.yaml

CKPT=$(ls -t checkpoints/mscoco-vqc10k-nosmooth/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_nosmooth.yaml -cp "$CKPT"
echo "finished $(date)"
