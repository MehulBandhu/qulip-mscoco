...
echo "node $(hostname) | vqc10k_l1 | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_l1.yaml

CKPT=$(ls -t checkpoints/mscoco-vqc10k-l1/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_l1.yaml -cp "$CKPT"
echo "finished $(date)"
