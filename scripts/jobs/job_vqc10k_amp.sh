...
echo "node $(hostname) | vqc10k_amp | $(date)"
python -u -m scripts.train -cfg configs/vqc10k_amp.yaml

CKPT=$(ls -t checkpoints/mscoco-vqc10k-amp/*/*/best.pt 2>/dev/null | head -1)
python -u -m scripts.benchmark -cfg configs/vqc10k_amp.yaml -cp "$CKPT"
echo "finished $(date)"
